...
const getPost = gql`
query getHomwork($postid: String!) {
...
export default class HomeworkDetailScreen extends Component {
    ...
    _commentHandle = () => {
        this.props.navigation.navigate('Comment', { postid: this.props.navigation.state.params.postid, type: 'homework' });
    }
        ...
        return (
            <View style={{ flex: 1 }}>
                <Query query={getPost} variables={{ postid: this.props.navigation.state.params.postid }} fetchPolicy="network-only" onCompleted={data => {
                    this.setState({ data: data.getHomework });
                    this.props.navigation.setParams({ PostUserName: data.getHomework.subject === undefined ? '' : data.getHomework.subject });
                }}>
                    {({ loading, error }) => {
                        ...
                        return <View style={{ flex: 1 }}>
                            <ScrollView overScrollMode='never'>
                                {this.state.data && this.state.data.pics ? <View>
                                ...
                                <View style={{ paddingHorizontal: 20, marginBottom: 60, marginTop: 10 }}>
                                    <View style={{}}>
                                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                            ...
                                            <Text style={{ fontSize: 14, lineHeight: 20 }}>{this.state.data.title}</Text>
                                        </View>
                                        <View style={{ height: 1, width: 100, marginTop: 4, backgroundColor: '#dbdbdb' }} />
                                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                            <View style={{ width: 4, height: 4, borderRadius: 2, backgroundColor: Colors.blue, marginRight: 4 }} />
                                            <Text style={{ fontSize: 14, lineHeight: 20 }}>{this.state.data.descriptions[0]}</Text>
                                        </View>
                                        <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                            <View style={{ width: 4, height: 4, borderRadius: 2, backgroundColor: Colors.red, marginRight: 4 }} />
                                            <Text style={{ fontSize: 14, lineHeight: 20 }}>{this.state.data.descriptions[1]}</Text>
                                        </View>
                                    ...
                                    <Text style={{ fontSize: 12, color: Colors.lightGray, fontWeight: 'bold', marginTop: 4 }}>
                                        {this.state.data.time} · 댓글 {this.state.data.commentNum}
                                    </Text>
                                </View>
                            </ScrollView>
