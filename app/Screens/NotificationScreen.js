...
import { Text, StyleSheet, View, Dimensions, ScrollView, Image, TouchableOpacity, ActivityIndicator, AsyncStorage, RefreshControl } from 'react-native'
...
mutation deleteNotification($postid: String!) {
    deleteNotification(postid: $postid) {
        createdAt
    }
}
`
...
export default class NotificationScreen extends Component {
    static navigationOptions = ({ navigation }) => {
        return {
            title: '공지',
            headerRight: (
                <BaseButton onPress={navigation.getParam('selectOption')} style={{ alignItems: 'center', justifyContent: 'center', height: 50, width: 50 }} >
    ...
    async componentDidMount() {
        this.props.navigation.setParams({ selectOption: this._selectPostOption });
                    ...
                    return <View style={{ flex: 1 }}>
                        <ScrollView showsVerticalScrollIndicator={false} overScrollMode='never' style={{ flex: 1 }} refreshControl={
                            ...
                            {data.listNotifications.items.length > 0 ? data.listNotifications.items.map((info, index) => <View key={index} style={{ width: WIDTH }}>
                                ...
                                {!this.state.lowDataMode || this.state.viewData[index] ? <View>
                                    <ScrollView pagingEnabled horizontal={true} showsHorizontalScrollIndicator={false} overScrollMode='never' style={{ width: '100%' }} scrollEventThrottle={16} onScroll={(event) => {
                                        ...
                                        {info.pics && info.pics.map((img, index2) =>
                                            <TouchableOpacity key={index2} activeOpacity={1} onPress={() => this.props.navigation.navigate('Photo', { image: info.pics, index: index2 })}>
                                ...
                                <View style={{ height: 40, flexDirection: 'row' }}>
                                    <BaseButton style={{ flex: 1, alignItems: 'center', justifyContent: 'center' }} onPress={() => this.props.navigation.navigate('Comment', { type: 'notification', postid: info.postid })}>
                                        {info.commentNum === 0
                                            ?
                        ...
                        <MyActionSheet
                            ...
                            onClicked={(data) => {
                                ...
                                if (data === 0) {
                                    this.props.navigation.navigate('NotificationPost');
                                } else {
                                    this.props.navigation.navigate('MyNotification', { userid: this.state.userid });
                                }
                            }}
