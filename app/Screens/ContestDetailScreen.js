...
export default class ContestDetailScreen extends Component {
    ...
    _commentHandle = () => {
        this.props.navigation.navigate('Comment', { postid: this.props.navigation.state.params.postid, type: 'contest' });
    }
    render() {
        return (
            <Query query={getPost} variables={{ postid: this.props.navigation.state.params.postid }} fetchPolicy="network-only" onCompleted={data => {
                this.setState({ data: data.getContest });
                this.props.navigation.setParams({ PostUserName: data.getContest.title });
            }}>
                {({ loading, error }) => {
                            ...
                            <View style={{ paddingHorizontal: 20, marginBottom: 60, marginTop: 10 }}>
                                <View style={{}}>
                                    <View style={{ flexDirection: 'row', alignItems: 'center' }}>
                                        ...
                                        <Text numberOfLines={1} style={{ fontSize: 14, lineHeight: 20 }}>{this.state.data.description}</Text>
                                    </View>
                                    {this.state.data.others && this.state.data.others.length > 0 && <View style={{ height: 1, width: 100, marginTop: 4, backgroundColor: '#dbdbdb' }} />}
                                    {this.state.data.others && this.state.data.others.map((info2, index) =>
                                        <View key={index}>
                                            <View style={{ flexDirection: 'row', alignItems: 'center', marginTop: 4 }}>
                                                <View style={{ width: 4, height: 4, borderRadius: 2, backgroundColor: index % 2 === 0 ? Colors.blue : Colors.red, marginRight: 4 }} />
                                                <Text style={{ fontSize: 14, lineHeight: 20 }}>{info2}</Text>
                                            </View>
                                        </View>
                                    )}
                                </View>
                                <Text style={{ fontSize: 12, color: Colors.lightGray, fontWeight: 'bold', marginTop: 4 }}>{this.state.data.time} · 댓글 {this.state.data.commentNum}</Text>
                            </View>
                        </ScrollView>
