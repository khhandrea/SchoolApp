...
import { Text, StyleSheet, View, ActivityIndicator, ScrollView, TextInput, TouchableWithoutFeedback, Dimensions, Platform, Image, AsyncStorage, Switch, Linking, ToastAndroid, Modal, Alert } from 'react-native'
...
mutation updateEvent($input: UpdateEventInput!, $adminid: String, $message: String, $resetComment: Boolean){
    updateEvent(input: $input, adminid: $adminid, message: $message, resetComment: $resetComment) {
        title
    }
}
`
...
class PostScreen extends Component {
    ...
    _postHandle = () => {
        if (this.state.version === null) {
            if (Platform.OS === 'android') {
                ToastAndroid.show('기달려주세요', ToastAndroid.SHORT);
            } else {
                Alert.alert("기달려주세요");
            }
            return;
        ...
        if (this.state.description === '' || this.state.title === '') {
            if (Platform.OS === 'android') {
                ToastAndroid.show('내용 or 제목을 입력해주세요', ToastAndroid.SHORT);
            } else {
                Alert.alert("내용 or 제목을 입력해 주세요");
            }
            return;
        }
    ...
    _upload2Server() {
        const input = {
            ...
            version: this.props.isPost ? this.state.version + 1 : this.state.version,
        }
        try {
            this.props.uploadMutation({ variables: { input: input, adminid: `${this.state.userid}(${Constants.deviceName})`, resetComment: this.props.isPost, message: `${this.props.isPost ? '님이 새 이벤트를 개시하였습니다.' : '님이 이벤트를 삭제하였습니다.'}` } }).then(response => {
            ...
            !this.state.loading
                ...
                <View style={{ flex: 1 }}>
                    <Query query={getEvent} fetchPolicy='network-only'
                        onCompleted={data => {
                            this.setState({ version: data.getEvent.version });
                            if (!data.getEvent.isOn && !this.props.isPost) {
                                Alert.alert('진행중인 이벤트가 없습니다');
                                this.props.navigation.goBack();
                                return;
                            }
                            if (!this.props.isPost) {
                                this.setState({ images: data.getEvent.pics, title: data.getEvent.title, description: data.getEvent.description, isComment: data.getEvent.isComment })
                            }
                        }}>
                        {({ loading }) => {
                            return null;
                    ...
                    <ScrollView showsVerticalScrollIndicator={false} overScrollMode={"never"} style={{ flex: 1 }}>
                        ...
                        <View style={{ borderBottomColor: '#dbdbdb', borderBottomWidth: 0.5 }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center', height: 40, marginLeft: 12 }}>
                                <View style={{ flex: 1, alignItems: 'center', flexDirection: 'row' }}><Text style={{ fontSize: 14, marginLeft: 10 }}>사진 (선택)</Text></View>
                            </View>
                        ...
                        <View style={{ borderBottomColor: '#dbdbdb', borderBottomWidth: 0.5, width: WIDTH }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center', height: 40, marginLeft: 12 }}>
                                <View style={{ flex: 1, alignItems: 'center', flexDirection: 'row' }}><Text style={{ fontSize: 14, marginLeft: 10 }}>제목</Text></View>
                            </View>
                            <View style={{ width: WIDTH, height: 40, paddingHorizontal: 20, alignItems: 'center', justifyContent: 'center' }}>
                                <TextInput placeholder='예) 삼행시 대회' value={this.state.title} style={{ borderColor: '#dbdbdb', borderWidth: 1, width: WIDTH - 40 }} onChangeText={text => this.setState({ title: text })} />
                            </View>
                        </View>
                        <View style={{ borderBottomColor: '#dbdbdb', borderBottomWidth: 0.5, width: WIDTH }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center', height: 40, marginLeft: 12 }}>
                                <View style={{ flex: 1, alignItems: 'center', flexDirection: 'row' }}><Text style={{ fontSize: 14, marginLeft: 10 }}>댓글</Text></View>
                            </View>
                            <View style={{ flexDirection: 'row', alignItems: 'center', width: WIDTH, height: 40 }}>
                                <Switch style={{ marginLeft: 12 }} value={this.state.isComment} onValueChange={value => this.setState({ isComment: value })} />
                            </View>
                        </View>
                        <View style={{ borderBottomColor: '#dbdbdb', borderBottomWidth: 0.5, width: WIDTH }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center', height: 40, marginLeft: 12 }}>
                                <View style={{ flex: 1, alignItems: 'center', flexDirection: 'row' }}><Text style={{ fontSize: 14, marginLeft: 10 }}>내용</Text></View>
                            </View>
                            <View style={{ width: WIDTH, height: 300, paddingHorizontal: 20, alignItems: 'center', justifyContent: 'center' }}>
                                <TextInput multiline={true} placeholder='예) 보정고로 3행시를 지어주세요' value={this.state.description} style={{ borderColor: '#dbdbdb', borderWidth: 1, width: WIDTH - 40, height: 280, textAlignVertical: 'top' }} onChangeText={text => {
            ...
            <Mutation mutation={POST_MUTATION} >
                {(createPost, { data }) => (
                    <PostScreen
                        ...
                        uploadMutation={createPost}
                        isPost={this.props.navigation.state.params.isPost}
                    />
                )
                }
            </Mutation>
