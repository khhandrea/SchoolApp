                            ...
                            return !this.state.loading || loading
                                ...
                                <View style={{ flex: 1 }}>
                                    <Query query={getPost} variables={{ postid: this.props.postid }} fetchPolicy='network-only'
                                        onCompleted={data => {
                                            this.setState({ subject: data.getHomework.subject, title: data.getHomework.title, descriptions: data.getHomework.descriptions, others: data.getHomework.others, images: data.getHomework.pics });
                                        }}>
                                        {({ loading }) => {
                                            return null;
                                        }}
    ...
    render() {
        return (
            <Mutation mutation={POST_MUTATION} >
                {(createPost, { data }) => (
                    <PostScreen navigation={this.props.navigation} uploadMutation={createPost} postid={this.props.navigation.state.params.postid} grade={this.props.navigation.state.params.grade} />
                )
                }
            </Mutation>
        )
