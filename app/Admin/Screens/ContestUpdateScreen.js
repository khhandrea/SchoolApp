...
const POST_MUTATION = gql`
    mutation updateContest($postid: String!, $input: UpdateContestInput!, $adminid: String){
        updateContest(postid: $postid, input: $input, adminid: $adminid) {
            postid
        }
    }
`
const getPost = gql`
query getContest($postid: String!) {
    getContest(postid: $postid) {
        title
        description
        others
        pics
    ...
    _postHandle = () => {
        if (this.state.description === '' || this.state.title === '') {
            if (Platform.OS === 'android') {
                ToastAndroid.show('(필수)내용을 입력해주세요', ToastAndroid.SHORT);
            } else {
    ...
    render() {
        return (
            !this.state.loading
                ...
                :
                <View style={{ flex: 1 }}>
                    <Query query={getPost} variables={{ postid: this.props.postid }} fetchPolicy='network-only'
                        onCompleted={data => {
                            this.setState({ images: data.getContest.pics, title: data.getContest.title, others: data.getContest.others, description: data.getContest.description })
                        }}>
                        {({ loading }) => {
                            return null;
                        }}
                    ...
                    <ScrollView showsVerticalScrollIndicator={false} overScrollMode={"never"} style={{ flex: 1 }}>
                        ...
                        <View style={{ borderBottomColor: '#dbdbdb', borderBottomWidth: 0.5, width: WIDTH }}>
                            ...
                            <View style={{ width: WIDTH, height: 40, paddingHorizontal: 20, alignItems: 'center', justifyContent: 'center' }}>
                                <TextInput placeholder='예) 보정 발명대회' value={this.state.title} style={{ borderColor: '#dbdbdb', borderWidth: 1, width: WIDTH - 40 }} onChangeText={text => this.setState({ title: text })} />
                            </View>
                        </View>
                        <View style={{ borderBottomColor: '#dbdbdb', borderBottomWidth: 0.5, width: WIDTH }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center', height: 40, marginLeft: 12 }}>
                                <View style={{ flex: 1, alignItems: 'center', flexDirection: 'row' }}><Text style={{ fontSize: 14, marginLeft: 10 }}>간단설명 (필수)</Text></View>
                            </View>
                            <View style={{ width: WIDTH, height: 40, paddingHorizontal: 20, alignItems: 'center', justifyContent: 'center' }}>
                                <TextInput placeholder='예) 발명 아이디어를 제출하는 대회' value={this.state.description} style={{ borderColor: '#dbdbdb', borderWidth: 1, width: WIDTH - 40 }} onChangeText={text => {
                        ...
                        <View style={{ borderBottomColor: '#dbdbdb', borderBottomWidth: 0.5, width: WIDTH }}>
                            <View style={{ flexDirection: 'row', alignItems: 'center', height: 40, marginLeft: 12 }}>
                                <View style={{ flex: 1, alignItems: 'center', flexDirection: 'row' }}><Text style={{ fontSize: 14, marginLeft: 10 }}>추가정보 (필수는 아니지만 날짜정도는 적어주세요)</Text></View>
                            </View>
                            <View style={{ width: WIDTH, height: 40, paddingHorizontal: 20, alignItems: 'center', justifyContent: 'center' }}>
                                <TextInput placeholder='예) 5월 11일 실시' value={this.state.others[0]} style={{ borderColor: '#dbdbdb', borderWidth: 1, width: WIDTH - 40 }} onChangeText={text => {
    ...
    render() {
        return (
            <Mutation mutation={POST_MUTATION} >
                {(createPost, { data }) => (
                    <PostScreen navigation={this.props.navigation} postid={this.props.navigation.state.params.postid} uploadMutation={createPost} />
                )
                }
            </Mutation>
        )
